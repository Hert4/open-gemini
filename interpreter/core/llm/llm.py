...
class Llm:
    ...
    def run(self, messages):
        ...
        # Enforce Gemini model
        if not model.startswith("gemini/"):
            # If user provided a model name without prefix, assume it's gemini
            # But if it looks like gpt-*, warn and switch?
            # For now, just prepend gemini/ if it doesn't look like a known provider
            if "/" not in model:
                model = f"gemini/{model}"
                self.model = model
            elif not model.startswith("gemini/") and not model.startswith("vertex_ai/"):
                 print(f"Warning: Model {model} is not a Gemini model. Switching to gemini/gemini-1.5-flash.")
...
Continuing...
    ...
    def load(self):
        ...
        # Enforce Gemini prefix if needed
        if not self.model.startswith("gemini/") and not self.model.startswith("vertex_ai/"):
